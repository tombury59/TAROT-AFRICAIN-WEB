  ...
  function broadcast(room) {
    ...
    if (room.spectators) {
      room.spectators.forEach(s => {
        if (s.socketId) {
          const view = buildPlayerView(room, s.id, true);
          io.to(s.socketId).emit('game_state', view);
        }
      });
    }
...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', ({ code, name, color }) => {
    ...
    if (room.state) {
      ...
      room.spectators.push({
        id: playerId,
        socketId: socket.id,
        name: name || `Spectateur ${room.spectators.length + 1}`,
        color: color || '#888888',
      });
      socket.join(code);
      socket.data.roomCode = code;
      ...
      socket.emit('joined', { code, playerId, isSpectator: true });
      broadcast(room);
      return;
    }
  ...
  socket.on('reconnect_room', ({ code, name }) => {
    ...
    if (room.spectators) {
      const existingSpec = room.spectators.find(s => s.name === name);
      if (existingSpec) {
        ...
        socket.data.isSpectator = true;
        socket.join(code);
        socket.emit('joined', { code, playerId: socket.id, isSpectator: true });
        broadcast(room);
        return;
      }
    }
